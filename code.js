...
function printThread(id){
  if (!id.includes("thread")) throw new Error("!thread_id")
...
function fixPendingMsgs() {
  const f = full_sheet_setup("msg-fixes",env('DEFAULT_SHEET_ID'))
  const l = f.getLastRow()
  for (let i = 2; i <= l; i++) {
    const [thread_id, old_msg, new_msg] = f.getRange(i,1,1,3).getValues()[0]
    // Logger.log(old_msg)
    if (thread_id == "" || !thread_id) continue
    let responded_to;
    let rows = get_thread_messages(thread_id).data
      .sort((a,b) => a.created_at < b.created_at ? -1 : 1)
      .map((item) => {return {
        time: Utilities.formatDate(new Date(item.created_at*1000), "America/Vancouver", "yyyy/MM/dd H:mm:ss"),
        from: item.role,
    ...
    rows.forEach((item,index) => {
      if (item.msg == old_msg) {
        Logger.log(rows[index-1].msg)
        f.getRange(i,4).setValue(rows[index-1].msg)
      }
    })
  }
}
...
function testyTest() {
  const input = `That's the right mindset! It's normal to have days when your appetite is higher, and it's crucial to listen to your body's signals. Just stay mindful of your hunger cues and make balanced choices. With your proactive attitude and the meal prep already done, you're set up for a fantastic day tomorrow. Keep up the fantastic work and go smash that workout! Remember, being in tune with your body and how you feel is essential. Keep moving forward; you're doing great!【24†source】.`
  Logger.log(addRandomNewLines(removeBlocks(input)))
}
...
function addRandomNewLines(text) {
  const maxNewLines = 3
  let newLines = 0
  ...
  return text.split('').map((char, index) => {
    ...
    if (punctuationMarks.includes(char) && Math.random() < 0.5-(newLines*0.1)) {
      justReturned = true
      newLines++
      return char + '\n\n';
    }
...
function msg_assistant(sheet_id, input_message, phone_number, name) {
  ...
  const reply = addRandomNewLines(removeBlocks(read_message(thread_id, messages[0]).content[0].text.value))
